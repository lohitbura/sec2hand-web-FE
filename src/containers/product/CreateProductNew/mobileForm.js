...
export default function MobileForm() {
  ...
  useEffect(() => {
    if (!brand) {
      fetchBrandListAPI({ type: "Mobile" }).then((res) => {
        setBrandList(res);
      });
    } else {
      fetchBrandListAPI({ type: "Mobile", brand_name: brand }).then((res) => {
        setModelList(res);
      });
    }
  }, [brand]);
  ...
  const submitForm = (values) => {
    ...
    form_data.append("ram", values.ram);
    form_data.append("rom", values.rom);
    form_data.append("back_camera", values.back_camera);
    form_data.append("front_camera", values.front_camera);
    form_data.append("earphone", values.earphone);
    form_data.append("charger_included", values.charger_included);
    form_data.append("box", values.box);
    form_data.append("bill", values.bill);

    for (let i = 0; i < images.length; i++) {
      form_data.append("images", images[i]);
    }

    createNewProductAPI("mobile", form_data);
  };
