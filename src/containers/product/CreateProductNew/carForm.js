...
export default function CarForm() {
  ...
  useEffect(() => {
    if (!brand) {
      fetchBrandListAPI({ type: "Car" }).then((res) => {
        setBrandList(res);
      });
    } else {
      fetchBrandListAPI({ type: "Car", brand_name: brand }).then((res) => {
        setModelList(res);
      });
    }
  }, [brand]);
  ...
  const submitForm = (values) => {
    ...
    form_data.append("fuel_type", values.fuel_type);
    ...
    createNewProductAPI("car", form_data);
  };
