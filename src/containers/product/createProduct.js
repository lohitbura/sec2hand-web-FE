...
class CreateProduct extends React.Component {
  ...
  fetchBrand = () => {
    ...
    if (brand !== "") {
      axios
        .post(brandListURL, { type: productCategory, brand_name: brand })
        .then((res) => [
          this.setState({
            modelList: res.data,
          }),
  ...
  createProduct = (url, category) => {
    const { price, model, images, km, color, fuel_type, type, year, brand } =
      this.state;
    ...
    axios
      .post(url, form_data, { headers: headers })
      .then((res) => {
